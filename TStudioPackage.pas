...
begin
  ...
  { Sanitize ProjectPath to prevent path traversal from crafted archives }
  if (Pos('..', Info.ProjectPath) > 0) or
     (Pos('/', Info.ProjectPath) > 0) or
     (Pos('\', Info.ProjectPath) > 0) or
     (Info.ProjectPath = '') then
  begin
    ErrorMsg := 'Invalid project path in package: ' + Info.ProjectPath;
    Exit(False);
  end;
